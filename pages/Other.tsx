...
const Other: React.FC = () => {
    ...
    const otherEmails = emails.filter(e => {
        if (e.category !== EmailCategory.Sonstiges) return false;
    ...
    return (
        <div>
            <h1 className="text-2xl font-semibold text-gray-900">Sonstiges</h1>
                ...
                <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        {otherEmails.length > 0 ? (
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">KI-Zusammenfassung</th>
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {otherEmails.map((email) => (
                                            <tr key={email.id}>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{email.senderName}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{email.subject}</td>
                                                <td className="px-6 py-4 text-sm text-gray-500 max-w-md truncate">{email.summary}</td>
                            ...
                            <EmptyState message="Keine E-Mails in der Kategorie Sonstiges gefunden, die den Kriterien entsprechen." />
                        )}
                    </div>
                </div>
            </div>
